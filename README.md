...
- 15 مصدرًا مسجّلًا في السجل المركزي.
- 54 حزمة ماركداون مصدرية موجَّهة للوكيل (دون احتساب ملف القالب `_template.md`).
- 42 ملفًا نصيًّا منظفًا أو عمليًّا تحت `data/text/`.
- 50 ملفًا خامًّا أو أثر اقتناء محفوظًا تحت `data/raw/` (دون احتساب `.gitkeep`).
...
11. وفي هذه الدفعة أضيف مصدر معجمي/بلاغي تأسيسي جديد هو `أساس البلاغة`: جُمعت صفحتا ويكي مصدر الكاملتان `الجزء الأول` و`الجزء الثاني` في raw واحد حجمه 351,727 bytes ونحو 33,952 كلمة عربية، واستُخرج text عملي من 28 جذرًا، ثم أُنتجت أربع حزم Markdown عن البيان والبلاغة، والأمانة والثبات، وبناء الفقرة، والصلة والاتباع. التقدير الصفحي نحو 90-110 صفحة مكافئة، وأربع حزم تحفظ قاعدة ~ملف لكل 20 صفحة بلا تفتيت زائد؛ والأولوية اللاحقة، إن استؤنف البناء، هي إضافة نطاق كبير جديد لا تلميع الحزم الحالية بلا حاجة.
