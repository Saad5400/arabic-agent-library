...
- 25 حزمة ماركداون مصدرية موجَّهة للوكيل (دون احتساب ملف القالب `_template.md`).
- 22 ملفًا نصيًّا منظفًا أو عمليًّا تحت `data/text/`.
- 33 ملفًا خامًّا أو أثر اقتناء محفوظًا تحت `data/raw/` (دون احتساب `.gitkeep`).
...
- حزم مكتملة الصياغة: 12
- مصادر مرشحة: 0
- مصادر مصطفّة للعمل القادم: 1
...
الأولوية الآن بعد إنجاز `ديوان أبي تمام` وفق دفعة raw واسعة ليست العودة إلى توسعات صغيرة متكررة، بل الانتقال حسب الخطة إلى آخر مصدر شعري queued وهو `ديوان البحتري`، مع الحفاظ على قاعدة الدفعات الخام الكبيرة والحزم المتوسطة الغنى.

ولهذا تتقدّم الأعمال الآتية:

1. اقتناء شاهد raw شعري واسع وحقيقي لـ `ديوان البحتري` بدل صفحة صغيرة أو قصيدة منفردة.
2. استخراج ملف text عملي واحد على الأقل من نطاق متصل أو دفعة صفحات عريضة.
3. تركيب حزم Markdown بعدد مبرر تحت قاعدة ~1 ملف لكل 20 صفحة تقريبًا.
4. إبقاء كل توسعٍ مقرونًا بنسبةٍ واضحةٍ وحدودٍ معلنة.
5. تأجيل أي عمل بنيوي ثانوي لا يخدم دفعة المصدر نفسها مباشرة.
