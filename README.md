...
الأولوية الآن بعد إنجاز مصادر الخطة المرحلية، وبعد إضافة `تاج العروس` كمصدر معجمي رابع بدفعة raw واسعة من شاهد Internet Archive، هي الانتقال إلى مراجعة الجودة النهائية على المشروع كله: التأكد من اتساق السجل، وعدالة عدد ملفات Markdown قياسًا إلى حجم الصفحات، وتحديث مذكرات الحالة دون تحويل المراجعة إلى عمل infra مستقل.

ولهذا تتقدّم الأعمال الآتية:

1. مراجعة كل مصدر `synthesized` في `data/registry/sources.json` للتأكد من وجود raw/text/markdown فعلًا.
2. مراجعة تناسب عدد ملفات Markdown مع حجم التغطية المصدرية، خصوصًا بعد الدفعات الشعرية والبلاغية الكبيرة.
3. توحيد صياغة `notes` في السجل حيث توجد بقايا حالة قديمة أو أرقام غير محدثة.
4. إبقاء كل حكم جودة مقرونًا بنسبةٍ واضحةٍ وحدودٍ معلنة.
5. تأجيل أي عمل بنيوي ثانوي لا يخدم مراجعة مصدرية ملموسة.
