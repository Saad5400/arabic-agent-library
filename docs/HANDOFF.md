...
- 139 text artifacts under `data/text/`, excluding `.gitkeep`
- 74 raw artifacts under `data/raw/`, excluding `.gitkeep`

### أحدث إنجاز مصدري بعد الإغلاق المرحلي

وُسِّع `لسان العرب` لابن منظور بدفعة معجمية جديدة غير مكررة من متن ويكي مصدر الحقيقي لا من صفحة الفهرس: جُمعت ثلاث صفحات حروف كبيرة (`حرف الثاء`، `حرف الجيم`، `حرف الخاء`) في raw واحد `data/raw/dict-lisan-al-arab-tha-jim-kha-letter-cluster.wikitext.txt` حجمه 816,295 bytes. استُخرج text عملي `data/text/dict-lisan-al-arab-tha-jim-kha-foundational-roots.txt` حول الثبات والتثبت، والثقل والوزن، والحذق والثقافة، والثمرة، والجبر بين الإصلاح والقهر، والخبر والاستخبار، والخبل والخبط والخبث. أضيفت وحدة عريضة واحدة إلى الملف الموحّد `data/markdown/dict-lisan-al-arab-complete-writing-foundations.md` بدل إنشاء ملفات صغيرة جديدة، لأن سياسة هذا المصدر بعد الدمج هي عدم تفتيت لسان العرب إلى حزم جذرية كثيرة. هذا العمل يؤكد مجددًا أن `data/raw/dict-lisan-al-arab.wikitext.txt` القديم فهرس/واجهة فقط، لا اقتناء متن معجمي كاف.
...
1. إذا كان المطلوب استمرار التوسيع، فأضف مصدرًا أو نطاقًا جديدًا كبيرًا وفق ترتيب يحدده `docs/MASTER_EXECUTION_CHECKLIST.md` بعد مرحلة الإغلاق، لا تعد إلى تلميع الحزم الحالية إلا عند ظهور خلل محدد. آخر توسيع كان معجميًا داخل `لسان العرب` عبر صفحات ويكي مصدر الحقيقية `حرف الثاء` و`حرف الجيم` و`حرف الخاء` في raw واحد حجمه 816,295 bytes؛ لذلك يُفضّل أن تكون الدفعة التالية من مصدر آخر أو نطاق كبير غير مكرر، أو امتداد تالٍ واضح لا يعيد تفصيل هذا النطاق. تجنب أيضًا إعادة تفصيل نطاقات `الصحاح` السابقة 20-1219 أو أبواب `القاموس المحيط` التي أُنجزت، وعنقودات `لسان العرب` الحالية، وأوراق `معجم مقاييس اللغة` 56-1105، ونطاقات `تاج العروس` 370-541، وأوراق `كتاب الصناعتين` 120-179، و`أدب الكاتب` 65-140، و`أساس البلاغة` الجزأين 1-2، واختيار `المقتضب` الحالي، وحزم `إصلاح المنطق` و`شرح ابن عقيل` و`شرح قطر الندى` و`الجمل في النحو` و`شرح شذور الذهب` 30-404.
