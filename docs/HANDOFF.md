...
- 222 final Markdown packs, excluding `data/markdown/_template.md`
- 126 text artifacts under `data/text/`, excluding `.gitkeep`
- 73 raw artifacts under `data/raw/`, excluding `.gitkeep`

### أحدث إنجاز مصدري بعد الإغلاق المرحلي

وُسِّع `معجم مقاييس اللغة` لابن فارس بدفعة معجمية تاسعة كبيرة غير مكررة من شاهد Internet Archive الكبير نفسه `20220809_Makaislugha`: استُخرجت الأوراق/OBJECTs 856-955، أي 100 ورقة إضافية متصلة تلي نطاق 756-855 مباشرة، وتبدأ بباب القاف وما بعدها في المضاعف/المطابق، ثم تمر بقطاع واسع من القاف والكاف وتفتح اللام إلى اللغو واللغب. الخام الكبير المعتمد محفوظ سابقًا في `data/raw/dict-maqayis-al-lugha.ia-djvu.xml` بحجم 33,729,915 bytes مع DjVuTXT وscandata؛ وأُنتج OCR وسيط جديد `data/text/dict-maqayis-al-lugha-leaves-856-955-ocr-extract.txt` حجمه 394,442 bytes، وtext عملي `data/text/dict-maqayis-al-lugha-leaves-856-955-writing-selection.txt`، ونص جامع `data/text/dict-maqayis-al-lugha-leaves-56-955-writing-selection.txt`. أضيفت خمس حزم Markdown عريضة عن القلة والثبات وبري العبارة؛ والقوت والقيادة والقدر؛ والقسر والكشف والقشر؛ والكمال والكناية والكدر والكفاية؛ واللهف والليّ واللصوق واللغو. صار مجموع تغطية `مقاييس اللغة` نحو 900 ورقة مصدرية وأربعًا وأربعين حزمة، وفق قاعدة ~ملف لكل 20 صفحة/ورقة بلا تفتيت جذور صغيرة.
...
1. إذا كان المطلوب استمرار التوسيع، فأضف مصدرًا أو نطاقًا جديدًا كبيرًا وفق ترتيب يحدده `docs/MASTER_EXECUTION_CHECKLIST.md` بعد مرحلة الإغلاق، لا تعد إلى تلميع الحزم الحالية إلا عند ظهور خلل محدد. آخر توسيع كان معجميًا داخل `معجم مقاييس اللغة` عبر شاهد IA `20220809_Makaislugha` للأوراق/OBJECTs 856-955؛ لذلك يُفضّل أن تكون الدفعة التالية من مصدر آخر أو نطاق كبير غير مكرر، أو امتداد تالٍ واضح لا يعيد تفصيل هذا النطاق. تجنب أيضًا إعادة تفصيل نطاقات `الصحاح` السابقة 20-1099 أو أبواب `القاموس المحيط` التي أُنجزت، وعنقود `لسان العرب` الحالي، وأوراق `معجم مقاييس اللغة` 56-955، ونطاقات `تاج العروس` 370-541، وأوراق `كتاب الصناعتين` 120-179، و`أدب الكاتب` 65-140، و`أساس البلاغة` الجزأين 1-2، واختيار `المقتضب` الحالي، وحزم `إصلاح المنطق` و`شرح ابن عقيل` و`شرح قطر الندى` و`الجمل في النحو` و`شرح شذور الذهب` 30-404.
