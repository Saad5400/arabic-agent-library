...
وُسِّع `معجم مقاييس اللغة` لابن فارس بدفعة معجمية خامسة كبيرة غير مكررة من شاهد Internet Archive الكبير نفسه `20220809_Makaislugha`: استُخرجت الأوراق/OBJECTs 456-555، أي 100 ورقة إضافية متصلة تلي نطاق 356-455 مباشرة، من أواخر الراء، وكتاب الزاء، وقطاع واسع من كتاب السين، وافتتاح كتاب الشين حتى مواد الشوق والشوك والشوه. الخام الكبير المعتمد محفوظ سابقًا في `data/raw/dict-maqayis-al-lugha.ia-djvu.xml` بحجم 33,729,915 bytes مع DjVuTXT وscandata؛ وأُنتج OCR وسيط جديد `data/text/dict-maqayis-al-lugha-leaves-456-555-ocr-extract.txt` حجمه 385,799 bytes / نحو 215,370 حرفًا، وtext عملي جديد `data/text/dict-maqayis-al-lugha-leaves-456-555-writing-selection.txt`، ثم النص الجامع المعتمد `data/text/dict-maqayis-al-lugha-leaves-56-555-writing-selection.txt`. أضيفت خمس حزم Markdown عريضة: `data/markdown/dict-maqayis-al-lugha-rajf-rijl-journey-return-and-zay-foundations.md`، و`data/markdown/dict-maqayis-al-lugha-zay-to-sin-ornament-falsification-and-disclosure.md`، و`data/markdown/dict-maqayis-al-lugha-sin-sickness-flow-elevation-and-coherence.md`، و`data/markdown/dict-maqayis-al-lugha-late-sin-covering-generosity-narration-and-splitting.md`، و`data/markdown/dict-maqayis-al-lugha-shin-splitting-support-testimony-and-distortion.md`. صار مجموع تغطية `مقاييس اللغة` نحو 500 ورقة وأربعًا وعشرين حزمة، وهو مناسب لقاعدة ~ملف لكل 20 صفحة/ورقة.
...
1. إذا كان المطلوب استمرار التوسيع، فأضف مصدرًا أو نطاقًا جديدًا كبيرًا وفق ترتيب يحدده `docs/MASTER_EXECUTION_CHECKLIST.md` بعد مرحلة الإغلاق، لا تعد إلى تلميع الحزم الحالية إلا عند ظهور خلل محدد. آخر توسيع كان معجميًا داخل `معجم مقاييس اللغة` عبر شاهد IA `20220809_Makaislugha` والأوراق/OBJECTs 456-555؛ لذلك يُفضّل أن تكون الدفعة التالية من مصدر آخر أو نطاق كبير لاحق غير مكرر، لا إعادة تفصيل أوراق `شرح شذور الذهب` 30-329 ولا باب الباء/الجيم من `القاموس المحيط` ولا نطاقات `الصحاح` 20-739 ولا عنقود `لسان العرب` الحالي ولا أوراق `معجم مقاييس اللغة` 56-555، مع تجنب تكرار العمل مباشرة على نطاقات `تاج العروس` 370-541 أو أوراق `كتاب الصناعتين` 120-179 أو `أدب الكاتب` 65-140 أو `أساس البلاغة` الجزأين 1-2 أو اختيار `المقتضب` الحالي أو حزم `إصلاح المنطق` و`شرح ابن عقيل` و`شرح قطر الندى` و`الجمل في النحو`.
