...
- 197 final Markdown packs, excluding `data/markdown/_template.md`
- 111 text artifacts under `data/text/`, excluding `.gitkeep`
- 73 raw artifacts under `data/raw/`, excluding `.gitkeep`

### أحدث إنجاز مصدري بعد الإغلاق المرحلي

وُسِّع `الصحاح تاج اللغة وصحاح العربية` للجوهري بدفعة معجمية ثامنة كبيرة غير مكررة من شاهد Internet Archive الكبير نفسه `civ-7220`: استُخرجت الأوراق/OBJECTs 860-979، أي 120 ورقة إضافية متصلة تلي نطاق 740-859 مباشرة، وتغطي ختام الغين وحرف الفاء وقطاعًا واسعًا من أوائل القاف حتى القيظ/القيض. الخام الكبير المعتمد محفوظ سابقًا في `data/raw/dict-al-sihah-al-jawhari.ia-djvu.xml` بحجم 47,839,854 bytes مع DjVuTXT وscandata؛ وأُنتج OCR وسيط جديد `data/text/dict-al-sihah-al-jawhari-leaves-860-979-ocr-extract.txt` حجمه 571,433 bytes / نحو 321,431 حرفًا، وtext عملي جديد `data/text/dict-al-sihah-al-jawhari-leaves-860-979-writing-selection.txt`، ثم النص الجامع المعتمد `data/text/dict-al-sihah-al-jawhari-leaves-20-979-writing-selection.txt`. أضيفت ست حزم Markdown عريضة: `data/markdown/dict-al-sihah-al-jawhari-ghina-ghanimah-futuwwa-and-fatwa.md`، و`data/markdown/dict-al-sihah-al-jawhari-opportunity-obligation-separation-and-eloquence.md`، و`data/markdown/dict-al-sihah-al-jawhari-vanishing-immediacy-quotation-and-precedence.md`، و`data/markdown/dict-al-sihah-al-jawhari-precedence-sparking-nearness-and-cutting.md`، و`data/markdown/dict-al-sihah-al-jawhari-peeling-division-intent-cutting-and-settling.md`، و`data/markdown/dict-al-sihah-al-jawhari-heart-contentment-strength-and-summer-heat.md`. صار مجموع تغطية `الصحاح` نحو 960 ورقة وثمانيًا وأربعين حزمة، وهو مناسب لقاعدة ~ملف لكل 20 صفحة/ورقة.
...
1. إذا كان المطلوب استمرار التوسيع، فأضف مصدرًا أو نطاقًا جديدًا كبيرًا وفق ترتيب يحدده `docs/MASTER_EXECUTION_CHECKLIST.md` بعد مرحلة الإغلاق، لا تعد إلى تلميع الحزم الحالية إلا عند ظهور خلل محدد. آخر توسيع كان معجميًا داخل `الصحاح تاج اللغة وصحاح العربية` عبر شاهد IA `civ-7220` والأوراق/OBJECTs 860-979؛ لذلك يُفضّل أن تكون الدفعة التالية من مصدر آخر أو نطاق كبير غير مكرر، أو امتداد بعيد/ضروري لا يعيد تفصيل هذا النطاق. تجنب أيضًا إعادة تفصيل باب الباء/الجيم/الحاء من `القاموس المحيط`، ونطاقات `الصحاح` 20-979، وعنقود `لسان العرب` الحالي، وأوراق `معجم مقاييس اللغة` 56-655، ونطاقات `تاج العروس` 370-541، وأوراق `كتاب الصناعتين` 120-179، و`أدب الكاتب` 65-140، و`أساس البلاغة` الجزأين 1-2، واختيار `المقتضب` الحالي، وحزم `إصلاح المنطق` و`شرح ابن عقيل` و`شرح قطر الندى` و`الجمل في النحو` و`شرح شذور الذهب` 30-404.
