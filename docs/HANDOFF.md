...
وُسِّع `شرح شذور الذهب في معرفة كلام العرب` لابن هشام بدفعة نحوية/أسلوبية ثالثة كبيرة غير مكررة من شاهد Internet Archive الكبير نفسه `hm4337`: استُخرجت الأوراق/OBJECTs 230-329، أي 100 ورقة إضافية متصلة تلي نطاق 130-229 مباشرة. الخام الكبير المعتمد محفوظ سابقًا في `data/raw/grammar-style-sharh-shudhur-al-dhahab.ia-djvu.xml` بحجم 13,933,142 bytes مع DjVuTXT وscandata؛ وأُنتج OCR وسيط جديد `data/text/grammar-style-sharh-shudhur-al-dhahab-leaves-230-329-ocr-extract.txt` حجمه 305,903 bytes، وtext عملي جديد `data/text/grammar-style-sharh-shudhur-al-dhahab-leaves-230-329-writing-foundations.txt`، ثم النص الجامع المعتمد `data/text/grammar-style-sharh-shudhur-al-dhahab-leaves-30-329-writing-foundations.txt`. أضيفت خمس حزم Markdown عريضة: `data/markdown/grammar-style-sharh-shudhur-al-dhahab-tamyiz-istithna-and-exceptive-precision.md`، و`data/markdown/grammar-style-sharh-shudhur-al-dhahab-negation-kay-hatta-and-purpose.md`، و`data/markdown/grammar-style-sharh-shudhur-al-dhahab-causative-fa-comitative-waw-and-governed-prepositions.md`، و`data/markdown/grammar-style-sharh-shudhur-al-dhahab-jussives-conditions-and-request-answers.md`، و`data/markdown/grammar-style-sharh-shudhur-al-dhahab-verb-governance-transitivity-and-complements.md`. صار مجموع تغطية `شرح شذور الذهب` نحو 300 ورقة وخمس عشرة حزمة، وهو مناسب لقاعدة ~ملف لكل 20 صفحة/ورقة.
...
1. إذا كان المطلوب استمرار التوسيع، فأضف مصدرًا أو نطاقًا جديدًا كبيرًا وفق ترتيب يحدده `docs/MASTER_EXECUTION_CHECKLIST.md` بعد مرحلة الإغلاق، لا تعد إلى تلميع الحزم الحالية إلا عند ظهور خلل محدد. آخر توسيع كان نحويًا/أسلوبيًا داخل `شرح شذور الذهب` عبر شاهد IA `hm4337` والأوراق/OBJECTs 230-329؛ لذلك يُفضّل أن تكون الدفعة التالية من مصدر آخر أو نطاق كبير لاحق غير مكرر، لا إعادة تفصيل أوراق `شرح شذور الذهب` 30-329 ولا باب الباء من `القاموس المحيط` ولا نطاق `الصحاح` 20-499 ولا عنقود `لسان العرب` الحالي ولا أوراق `معجم مقاييس اللغة` 56-455 ولا باب الجيم من `القاموس المحيط` ولا مسائل `درة الغواص` [1]-[25]، مع تجنب تكرار العمل مباشرة على نطاقات `تاج العروس` 370-541 أو أوراق `كتاب الصناعتين` 120-179 أو `أدب الكاتب` 65-140 أو `أساس البلاغة` الجزأين 1-2 أو اختيار `المقتضب` الحالي أو حزم `إصلاح المنطق` و`شرح ابن عقيل` و`شرح قطر الندى` و`الجمل في النحو`.
